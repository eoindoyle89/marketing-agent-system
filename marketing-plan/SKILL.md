---
name: marketing-plan
description: When the user needs a comprehensive marketing plan for a client, a company they advise, or their own product. Also use when the user mentions "marketing plan," "growth plan," "GTM plan," "go-to-market plan," "AARRR plan," "90-day marketing plan," "12-month marketing roadmap," "fractional CMO plan," or "fCMO plan." Generates an exhaustive 13-section plan structured by AARRR (Acquisition, Activation, Retention, Referral, Revenue), customized to the client's current budget, team, and stage, mapped to future funding milestones, cross-referenced with the 139-idea marketing-ideas library and an embedded 17-section current-state audit rubric, with a full marketing operations stack showing which skills and MCP/API integrations execute each part. Outputs a Notion-paste-ready markdown document. For positioning and ICP context before planning, run marketing-system-setup. For stage-specific deep work, see onboarding, signup, emails, referrals, pricing.
---
...
Also load `channel-rules.md`, `proof-points.md`, `case-studies.md`,
`brand-voice.md`, `visual-identity-and-assets.md`, `qa-policy.md`, and
`sources.md` when present. Ask for missing budget, team, funding-stage, and
timeframe details before drafting the plan. The plan is a proposal for human
approval; keep claims inside `proof-points.md` limits.
...
On invocation, the skill reads `marketing-plans/{client-slug}/progress.md` in the current working folder and resumes based on the state machine documented in `references/methodology.md` Step 1.1.2 (fresh → INIT → REVIEW → FINALIZE → finalized). Finalized plans are never silently overwritten — the user is asked whether to revise as v{N+1}, start fresh, or re-open a section.
...
2. **`marketing-system-setup`** — Provisions the durable `.agent-context/` company store (positioning, ICP, voice, proof). Run this first; Section 2 (Strategic frame) builds on it.
...
`marketing-plans/{client-slug}/final_plan.md` in the current working folder.
...
- **`product-marketing`** — Run first. Captures positioning, ICP, voice in the connected vault's product-marketing note so every section of the plan references the same foundation, whichever project you're in.
