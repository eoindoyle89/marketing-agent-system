...
Persist each loop's state in a file under `.agent-context/loops/` in the
company context store. One state file per loop:

```
.agent-context/loops/<loop-name>.json     # the loop's memory
.agent-context/loops/<loop-name>.log      # append-only run log
```
