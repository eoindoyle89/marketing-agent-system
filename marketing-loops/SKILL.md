...
Also load `offers-pricing-and-packaging.md`, `channel-rules.md`,
`qa-policy.md`, and `sources.md` when present. Ask for missing
measurement capability, loop ownership, and tooling details before designing
loops. Loops are proposals with human approval gates at every
externally-visible step; do not run, send, or publish loop actions.
...
- **Agent runtimes with native scheduling** — use the runtime's own self-paced loop or scheduled-task mechanism where one exists.
