...
Before applying mental models, read `../shared/context-read-protocol.md` and
follow it.

Use `.agent-context/INDEX.md` as the primary company context source when a
context store is available. Treat marketing-psychology work as requiring the `Content strategy, copy, social,
...
`offers-pricing-and-packaging.md`, and `sources.md` when present. Ask for missing audience
and decision-context details before applying mental models. Treat
behavioural-science effects as directional heuristics; do not turn them into
public factual claims unless approved through `proof-points.md`.
