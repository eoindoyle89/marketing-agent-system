...
are complete. `start-here`, `guide`, `context-update`,
`marketing-orchestrator`, `campaign-planner`, and `marketing-ux` are new skills
created directly for this repo, not packaged Personal Vault skills. Do not push
future packaged or newly-created skills until:
...
| Skill | Source folder | Status | Provenance flag | Packaging notes |
|---|---|---|---|---|
| `start-here` | Created in repo | Packaged locally, not published | Original / no direct installed match found | Adds the first-run onboarding experience for nontechnical users; explains what the system is, credits Eoin Doyle matter-of-factly, explains context storage and approval boundaries, then routes into `marketing-system-setup` and asks the first storage question without creating anything. |
| `guide` | Created in repo | Packaged locally, not published | Original / no direct installed match found | Adds a recallable usage guide prompt; routes to `start-here` when no context store exists, otherwise points users to `marketing-orchestrator` as the default front door and summarizes direct-skill exceptions and approval boundaries. |
