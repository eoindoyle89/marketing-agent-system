...
Before designing retention or cancel flows, read `../shared/context-read-protocol.md` and
follow it.

Use `.agent-context/INDEX.md` as the primary company context source when a
context store is available. Treat churn-prevention work as requiring the `CRO, signup, onboarding,
paywalls, churn` task family.
...
Also load `campaign-history.md`, `marketing-operations.md`,
`proof-points.md`, `qa-policy.md`, and `sources.md` when present. Ask for missing churn
baselines, billing provider, entitlement rules, and save-offer discount
authority before recommending flows or offers. Recommend only; do not change
live cancel flows, billing, or offers.
...
Use connected billing, retention, or analytics tools only when they are
available in the current runtime. If no connector is available, ask the user
for exports or platform settings before making account-specific
recommendations.
