...
Before planning community programs, read `../shared/context-read-protocol.md` and
follow it.
...
context store is available. Treat community work as requiring the `Content strategy, copy, social,
...
Also load `proof-points.md`, `positioning-and-icp.md`,
`campaign-history.md`, `qa-policy.md`, and `sources.md` when present. Ask for missing platform
ownership, moderation capacity, and governance details before recommending a
community program. Recommend and draft only; do not post, invite, or moderate
on live channels.
