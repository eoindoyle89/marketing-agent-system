...
Before drafting sales collateral, read `../shared/context-read-protocol.md` and
follow it.
...
context store is available. Treat sales-enablement work as requiring the `Sales enablement, outbound,
...
Also load `customer-research-and-voc.md`,
`visual-identity-and-assets.md`, `goals-metrics-and-funnel.md`,
`marketing-operations.md`, `qa-policy.md`, and `sources.md` when present. Ask for missing
sales-motion, deal-stage, objection, and asset-format details before drafting
collateral. Keep every claim inside `proof-points.md` limits and customer
evidence inside recorded permissions; draft for human approval only.
...
Use connected partner-management tools only when they are available in the
current runtime. If no connector is available, ask the user for exports or
settings before making account-specific recommendations. Example platform:
