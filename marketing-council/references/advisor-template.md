...
Copy this structure to add an advisor to the bench. Save custom advisors to `.agent-context/advisors/<kebab-name>.md` in the company context store (not inside the skill folder) so they survive skill updates.
...
**Seating a custom advisor:** mention them by name when convening ("seat my advisor Maria on this council"). The agent loads the file from `.agent-context/advisors/` and treats it like any bench dossier, including the grounding rules — no fabricated quotes, no invented endorsements.
