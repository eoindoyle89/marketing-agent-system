...
Before producing imagery, read `../shared/context-read-protocol.md` and
follow it.
...
context store is available. Treat image work as requiring the `Image and video production` task family.
...
`campaign-history.md`, and `sources.md` when present. Ask for missing
brand-asset locations, usage rights, model and licence constraints, and channel
specs before generating or recommending imagery. Confirm rights before any
asset is proposed for external use.
