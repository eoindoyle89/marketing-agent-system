...
Before constructing offers, read `../shared/context-read-protocol.md` and
follow it.

Use `.agent-context/INDEX.md` as the primary company context source when a
context store is available. Treat offer work as requiring the `Pricing, offers, launch,
...
`brand-voice.md`, `qa-policy.md`, and `sources.md` when present. Ask for missing discount and
guarantee approval authority, margin boundaries, and effective dates before
constructing offers. Offer designs are proposals; record approved offers,
terms, and boundaries in `offers-pricing-and-packaging.md`, and keep every
claim inside `proof-points.md` limits.
