...
Before planning video production, read `../shared/context-read-protocol.md` and
follow it.
...
context store is available. Treat video work as requiring the `Image and video production` task family.
...
`proof-points.md`, and `sources.md` when present. Ask for missing
footage sources, talent and music rights, tool availability, and channel specs
before planning production. Treat tool and vendor capabilities in references as
time-sensitive; verify before relying on them.
