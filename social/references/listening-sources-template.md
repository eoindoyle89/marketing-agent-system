...
Copy this file to `.agent-context/attachments/listening-sources.md` and fill in the brackets. Record it in `.agent-context/sources.md`. Agents use it when running the [listening workflow](listening.md).
...
Search across all platforms. Agents run these through approved public sources on the [daily loop](listening.md#the-daily-triage-loop).
...
URLs the agent may inspect after user approval.
...
- For LinkedIn and X, use an approved browser session only after the user confirms access scope
