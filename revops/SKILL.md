...
Also load `customer-research-and-voc.md`, `campaign-history.md`,
`qa-policy.md`, and `sources.md` when present. Ask for missing
system-of-record, lifecycle-stage definitions, tool-stack access, and
data-ownership details before recommending scoring, routing, or automation
changes. Recommend only; do not change live CRM records, automations, or
routing.
...
Use connected CRM, enrichment, or automation tools only when they are
available in the current runtime. If no connector is available, ask the user
for exports, screenshots, or settings before making account-specific
recommendations. Common platforms:
