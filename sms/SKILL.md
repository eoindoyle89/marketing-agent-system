...
Before planning SMS programs, read `../shared/context-read-protocol.md` and
follow it.

Use `.agent-context/INDEX.md` as the primary company context source when a
context store is available. Treat SMS work as requiring the `Content strategy, copy, social,
...
`case-studies.md`, `qa-policy.md`, and `sources.md` when present. Ask for missing consent
records, opt-in method, jurisdictions, quiet-hours policy, platform, and volume
details before recommending sends or sequences. Draft only; do not send
messages or change live automations.
...
Use connected SMS-platform tools only when they are available in the current
