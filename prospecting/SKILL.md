...
Before building prospect lists, read `../shared/context-read-protocol.md` and
follow it.
...
context store is available. Treat prospecting work as requiring the `Sales enablement, outbound,
...
Also load `case-studies.md`, `goals-metrics-and-funnel.md`,
`marketing-operations.md`, `qa-policy.md`, and `sources.md` when present. Ask for missing
target-market definition, data-source access and permissions, lawful-basis and
compliance constraints for the target jurisdictions, and volume details before
building lists. Research and recommend only; do not contact prospects, and do
not collect data in ways that violate platform terms or privacy law — see
...
Pull from `.agent-context/positioning-and-icp.md` and
`.agent-context/customer-research-and-voc.md` when a context store exists.
Otherwise, gather:
...
- **ICP description** — pull from `.agent-context/positioning-and-icp.md` if present
...
Use connected data, enrichment, or research tools only when they are available
in the current runtime. If no connector is available, ask the user for exports
or tool access before making source-specific recommendations. Common tools:

| Tool | Best For |
|------|----------|
| **Apollo** | B2B / SaaS firmographic + contact discovery |
| **Clay** | Multi-source enrichment + waterfall |
| **Clearbit** | Email-to-company enrichment |
| **ZoomInfo** | Enterprise B2B contact + intent |
| **Hunter** | Email pattern + verification |
| **Snov** | Email finder + verifier |
| **Truelist** | Email deliverability validation |
| **Outreach** | Sales engagement (post-list) |
| **RB2B** | Visitor identification (warm intent) |
| **GitHub** | Stargazers/forks/watchers as developer-intent signal |
| **Firecrawl** | Single-target site extraction (prospect's own website) |
| **Browserbase** | Real-browser site research when rendering or interaction needed |
